...
const DisplayFormMethod = ({ data }) => {
  let jsonData;

  // State variables for each section
  const [isMainDetailsMinimized, setIsMainDetailsMinimized] = useState(true);
  const [isPermanentAddressMinimized, setIsPermanentAddressMinimized] = useState(true);
  const [isPresentAddressMinimized, setIsPresentAddressMinimized] = useState(true);
  const [isGeneralInformationMinimized, setIsGeneralInformationMinimized] = useState(true);
  const [isIdentificationDetailsMinimized, setIsIdentificationDetailsMinimized] = useState(true);
  const [isVisaDetailsMinimized, setIsVisaDetailsMinimized] = useState(true);
  const [isComplianceMinimized, setIsComplianceMinimized] = useState(true);
  const [isEmergencyContactMinimized, setIsEmergencyContactMinimized] = useState(true);
  ...
  const identitytype = (value) => {
    ...
    if (value.includes('(changed)')) {
      ...
      switch (value) {
            case "99": 
            return "Non Individual Group "
            case "01": 
            return "National Identity No. "
            case "02": 
            return "NID (1"
            case "03": 
            return "Passpor"
  ...
  const valueMappings = {
    ...
    'CustRiskImp': (value) => CustRiskImp(value),
...
export default DisplayFormMethod;